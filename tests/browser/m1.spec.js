...
test('reload never resurrects active state from history',async({page})=>{await page.goto('/tr');await expect(page.locator('main')).toHaveAttribute('aria-busy','false');await page.reload();await expect(page.locator('section')).toHaveAttribute('data-view','interrupted');await expect(page.getByRole('button',{name:'Göreve dön',exact:true})).toBeVisible();});
