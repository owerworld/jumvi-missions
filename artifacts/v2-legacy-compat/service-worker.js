 ...
 * ═══════════════════════════════════════════════════════════════════════════ */
const CACHE_NAME = "jumvi-missions-v254-v2compat1";
const CORE_ASSETS = [
  "/",
  "/index.html",
  ...
  "/assets/ui/missions/21-middle-defender.webp",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => /^jumvi-missions-v[0-9]+(?:-v2compat[0-9]+)?$/.test(k) && k !== CACHE_NAME).map((k) => caches.delete(k)))
    )
  );
  self.clients.claim();
});
