...
export function missionCopy(record,locale,labels,presentation){
 ...
 return {...labels,...c,mission:c.title||c.mission,indoor:c.indoor||'',materials:c.materials||equipmentText(record,locale),setup:c.setup||'',toss:steps[0]||'',catch:steps.slice(1).join(' '),safe:c.safety||c.safe||'',activeCopy:pilot?.activeCopy?.[locale]||c.activeCopy||steps[0],activeCatch:c.activeCatch||(locale==='tr'?'Mavi ön yüzle yakala.':'Catch on the blue front.'),canonicalSteps:steps,frames:pilot?.frames||record.art.frames,hero:pilot?.hero,activeArt:pilot?.active,stepsTitle:c.stepsTitle||(locale==='tr'?'Nasıl oynanır':'How to play')};
}
