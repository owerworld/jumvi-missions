...
async function boot(){
 ...
 let record=selected==='m25'?fixture:await load(`missions/${selected}.json`,catalog.missions.find(m=>m.id===selected).sha256),cp=missionCopy(record,locale,labels,presentation),loadGeneration=0,pendingMission=null;
 let state=initialState({documentId:crypto.randomUUID(),locale,mission:{id:record.id,mechanicsVersion:record.mechanicsVersion,contentVersion:record.contentVersion,ready:false}}),router;
 ...
 function render({preserveFocus=false}={}){const priorPosition=navigation.capture();const focus=preserveFocus&&app.contains(document.activeElement)?{id:document.activeElement.id,start:document.activeElement.selectionStart,end:document.activeElement.selectionEnd}:null;const captured=state,send=(type,detail={},push=true)=>dispatch({type,...detail,revision:captured.revision,documentId:captured.documentId},push);
  app.replaceChildren(views[state.screen]({state,ui,cp,assetManifest,catalog,audio,send,personal:personal.model,isSaved:personal.isSaved(state.report)}));const p=document.createElement('p');p.className='play-notice';p.setAttribute('role','status');p.setAttribute('aria-live','polite');app.append(p);app.setAttribute('aria-busy','false');placeUtilities();navigation.render(state,priorPosition);refreshAudio();if(focus?.id){const target=document.getElementById(focus.id);target?.focus({preventScroll:true});if(target?.type==='text'&&focus.start!==null)target.setSelectionRange(focus.start,focus.end);}
 }
 function placeUtilities(){const textSize=parseFloat(getComputedStyle(document.documentElement).fontSize),compact=innerWidth<24.375*textSize;app.classList.toggle('compact-controls',compact);const section=app.querySelector('[data-view=entry]'),support=section?.querySelector('.entry-support');if(!support)return;support.classList.toggle('compact',compact);(compact?section.querySelector('h1'):section.querySelector('.setup')).before(support);}
 ...
 router=installRouter(()=>state,(type,d={},push=true)=>dispatch({type,...d,revision:state.revision,documentId:state.documentId},push),{capture:()=>navigation.capture(),restore:async({route,position})=>{
  ...
  try{const meta=catalog.missions.find(m=>m.id===route.missionId);if(!meta)return;const next=record.id===meta.id?record:await load(`missions/${meta.id}.json`,meta.sha256);if(gen!==loadGeneration||rev!==state.revision)return;
   ...
   try{sessionStorage.setItem(storageName('jumvi.selected-mission.v1'),next.id);}catch{}state={...state,loadingAssets:false};void retryAssets();
