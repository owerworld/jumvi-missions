...
export function initialState({documentId, locale='tr', mission=null}={}) {
 return {documentId,locale,mission,revision:0,screen:'entry',round:null,report:null,record:recordBoundary,media:initialMedia(),returnContext:null,unknown:false};
}
export function transition(s,e) {
 ...
 case 'START': case 'REPLAY':
  ...
  n={...s,screen:'active',round:createRound(e.id,s.mission),report:null,unknown:false};break;
 case 'STOP':
  if (!s.round || !['active','help'].includes(s.screen) || s.round.state!=='active') return s;
  n={...s,screen:'stopped',round:changeRound(s.round,'stopped'),returnContext:null};break;
 case 'RESUME':
  ...
  n={...s,screen:'active',round:changeRound(s.round,'active')};break;
 case 'INTERRUPT':
 ...
 case 'REPORT': {
  ...
  const report=explicitReport(e.id,s.mission,s.round,e.value,s.screen==='report'?s.report:null);
  if(!report) return s;
  n={...s,screen:'report',report};break;
 }
