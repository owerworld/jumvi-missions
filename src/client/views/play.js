...
export function playView(name,ctx) {
 ...
 if(name==='entry'){
  brand();const sup=support();sup.classList.add('entry-support');s.append(sup,heading(cp.mission),text(cp.goal,'instruction goal'),(cp.frames?frameArt(cp.frames[0]):art('seated-editorial-v1',cp.step1body+' '+cp.toss+' '+cp.catch)),text(cp.materials,'materials'),text(cp.indoor,'muted indoor'));
  ...
  const contact=el('div',undefined,'active-contact');contact.append(cp.frames?frameArt(cp.frames[Math.min(1,cp.frames.length-1)]):art('contact-alpha-v1',cp.step2body,'contact-art'),text(cp.activeCatch,'instruction'));s.append(contact,text(cp.goal,'instruction goal'));
