...
const inputs=[join(root,'index.html'),...files(join(root,'src/client')),...files(join(root,'content')),...files(join(root,'src/offline'))];
...
const mime=p=>p.endsWith('.html')?'text/html':p.endsWith('.js')?'text/javascript':p.endsWith('.css')?'text/css':p.endsWith('.json')?'application/json':p.endsWith('.webp')?'image/webp':p.endsWith('.ttf')?'font/ttf':'application/octet-stream';
...
const precache=publicFiles.filter(f=>f.path.endsWith('.html')||f.path.includes('/client/')||f.path.includes('/assets/fonts/')||['/content/catalog.json','/content/asset-manifest.json','/content/ui/tr.json','/content/ui/en-US.json'].some(p=>f.path.endsWith(p))).map(f=>f.path);
