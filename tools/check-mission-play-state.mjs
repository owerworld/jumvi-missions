...
if (wanted("T29")) await session(async (t) => {
  ...
  if (!webgl || !hookPresent) {
    record("T29", "3D hub completion", "SKIP",
      `not exercised here — webgl2=${webgl}, openMissionFromHub=${hookPresent}. The hub loads lazily behind a flag and needs a real GL context; it is untested this round, not passing.`);
    return;
  }
